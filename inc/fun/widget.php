...
abstract class puockWidgetBase extends WP_Widget
{
    ...
    public function get_common_widget_header($instance)
    {
        ...
        <div class="pk-widget p-block <?php echo $instance['classes'] ?>">
            <?php if ($show_title): ?>
                ...
                puock-text pb-2"><i class="<?php echo $this->get_icon($instance['icon']) ?> mr-1"></i>
    ...
    {
        return $this->merge_common_fields(array_merge(
            array(
...
add_action('widgets_init', function () {
    register_widget('puockHotCommentPost'); });
...
class puockReadPerson extends puockWidgetBase
{
    ...
    function get_fields()
    {
        return $this->merge_common_fields(
            array(
                array('id' => 'title', 'strip' => true, 'val' => $this->title),
                    ...
                    <?php foreach ($authors as $author): ?>
                        <div class="col col-12 col-lg-6 pl-0">
                            <div class="p-2 text-truncate text-nowrap">
                                <a href="<?php echo empty($author->url) ? 'javascript:void(0)' : pk_go_link($author->url) ?>"
                                    class="a-link" <?php echo empty($author->url) ? '' : 'target="_blank"' ?> rel="nofollow">
                                    <img <?php echo pk_get_lazy_img_info(get_avatar_url($author->mail), 'md-avatar') ?>
...
add_action('widgets_init', function () {
    register_widget('puockReadPerson'); });
                ...
                <div class="min-comments t-md">
                    <?php foreach ($comments as $comment): ?>
                        <div class="comment t-md t-line-1">
                            ...
                            <span class="c-sub t-w-400">
                                <?php echo strip_tags(convert_smilies($comment->comment_content), ['img']) ?>
                            </span>
                        </div>
                    <?php endforeach; ?>
                </div>
...
add_action('widgets_init', function () {
    register_widget('puockNewComment'); });
...
add_action('widgets_init', function () {
    register_widget('puockStrongText'); });
                    ...
                    <div class="<?php if (!$this->is_checked($instance['hide_title'])): ?>mt20<?php endif; ?>">
                        <form class="global-search-form" action="<?php echo home_url() ?>" method="get">
                            <div class="input-group">
                                <input type="text" name="s" class="form-control t-md"
                                    placeholder="<?php echo $instance['pl'] ?>">
                            </div>
                        </form>
                    </div>
                </div>
...
add_action('widgets_init', function () {
    register_widget('puockRandomPost'); });
...
add_action('widgets_init', function () {
    register_widget('puockAboutAuthor'); });
...
class puockCategory extends puockWidgetBase
{
    ...
    function widget($args, $instance)
    {
        ...
        foreach ($cats as $cat) { ?>
             ...
             d-inline-block w-100" title="<?php echo $cat->name ?>">
                            <?php echo $cat->name ?>
                        </a>
                    </div>
                <?php }
...
add_action('widgets_init', function () {
    register_widget('puockTagHitokoto'); });
